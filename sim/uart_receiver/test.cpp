...
void write_char(augmented_module<Vuart_receiver> &receiver, char value,
  ...
  // Start bit
  receiver->uart_rx = 0;
  receiver.cycle(wait_time + wait_time_offset, {data_is_invalid});

  // Data bits
  for (auto bit = 0; bit < 8; ++bit) {
    receiver->uart_rx = (value >> bit) & 0b1;
    receiver.cycle(wait_time, {data_is_invalid});
  }

  // Stop bit
  receiver->uart_rx = 1;
  receiver.cycle(wait_time - wait_time_offset);
}
